...
(function (cellula) {
    ...
    function Events (){
        ...
        this.emit = function(events){
            var follower, other, calls, event, pack, args = util.slice.call(arguments, 1, arguments.length); // slice could be optimized
            // all
            ...
            if(util.isArray(events)){
                while((event = events.shift())){
                    ...
                    util.each(other, function(v){ v.receiver.apply(v, [pack].concat(args)); });
                }
            }
        };
