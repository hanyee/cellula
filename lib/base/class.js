    ...
    global.Class = function(){
        ...
        var _class = function(){
            if(!this.constructor.__initializing__){
                ...
                this.on = function (events, callback, context) { // this.listen
                    ...
                    while (event = events.shift()) {
                        ...
                        if (list) {
                            for (i = 0, length = list.length; i < length; i += 2) {
                                list[i].apply(list[i + 1] || this, rest.concat([event])); //list[i].apply(list[i + 1], rest);
                            }
                        }
                ...
                // create a new proto
                var newProtoFlag = _args[2] || _args[1];
                if(newProtoFlag === 'NEW'){
                    var tmp, p, q;
                    for(p in this){
                        // object
...
/*
                for(var name in proto){
                    prop[name] = name !== 'constructor' && typeof prop[name] === "function" && typeof _super[name] === "function" && fnTest.test(prop[name]) ?
                        (function(name,fn){
                            return function() {
                                var tmp = this._super;
                        ...
                        })(name, prop[name]) : (prop[name]?prop[name]:proto[name]);

                    // object
                    if(objTest.test(toString.call(prop[name]))) prop[name] = (function(){
                        var tmp = {}, p; // limited for hasOwnProperty?
                        for(p in prop[name]){ tmp[p] = prop[name][p]; }
                        return tmp;
                    })();

                    // array
                    if(arrTest.test(toString.call(prop[name]))) prop[name] = prop[name].slice();
                }
                this.prototype.constructor = this;
*/
